# Trade Clash (fiction) — the SYNTHETIC fixture the tests use

**This is invented content, and that is its job.** Three trade blocs sharing a
shipping lane: Ferrox, Cindra and Okuma. None of them exist.

The real Trade Clash IP lives in `fixtures/tradeclash`, compiled from the
product's own brand document by `npm run fixture`. Demos and the video use that
one. The tests use this one, on purpose:

- A pipeline test should fail when the **pipeline** breaks, not when an owner
  edits their brand document. A test fixture wants to be frozen; a demo fixture
  wants to be true. Those are different jobs and they now have different
  directories.
- This fixture ships a complete `hints.json` — two live arcs and a directed
  three-way relationship graph — so the ingest and demo-beat tests have a
  storyline to thread a post onto. The real fixture deliberately ships none of
  that, because a real brand document does not contain it.

Do not demo with this. `npm run prove` will warn you if you try: hints
containing arcs leave a Mind nothing to add, and the comparison reads as a tie.

## Field reference

Kept because it is the clearest description of the fixture format anywhere in
the repo. It reads as "what you must replace" because it was written when this
was the only Trade Clash fixture; the replacing has since been done properly in
`fixtures/tradeclash`.

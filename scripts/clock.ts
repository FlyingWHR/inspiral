...
import { startPatrol, DEFAULT_PATROL, predictedCachedRate, PATROL_PROFILES } from "../src/tick/patrol.js";
...
/**
 * THE PATROL. `--patrol 0` turns it off; otherwise the number is the MEAN GAP
 * IN HOURS between visits for each profile, drawn from an exponential.
 *
 * See src/tick/patrol.ts for why it is memoryless, why the ids are `sim_`
 * prefixed, and why it is a control arm rather than a source of evidence.
 */
const PATROL_MEAN_H = flag("patrol", DEFAULT_PATROL.meanGapHours);
...
async function main(): Promise<void> {
  ...
  console.log(`  │  patrol      ${PATROL_MEAN_H > 0
    ? `${PATROL_PROFILES.length} synthetic profiles, Exp(mean ${PATROL_MEAN_H}h) gaps` +
      `  -- predicted cached-greeting rate ` +
      `${predictedCachedRate(PATROL_MEAN_H, EVERY_MIN / 60).toFixed(4)}`
    : "off"}`);
  ...
  /**
   * Independent of the tick timer, deliberately. Phase-locking them would drive
   * the hollow-return rate to zero and destroy the falsification test that
   * depends on the two schedules being independent.
   */
  const stopPatrol =
    PATROL_MEAN_H > 0
      ? startPatrol({ ctx, cfg: { ...DEFAULT_PATROL, meanGapHours: PATROL_MEAN_H,

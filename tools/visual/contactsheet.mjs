#!/usr/bin/env node
/**
 * VENDORED FROM thrixel/build-world (Apache-2.0) -- see LICENSE-Apache-2.0.txt.
 * https://github.com/thrixel/build-world
 *
 * Modified by Inspiral: the `parseArgs` import now points at the local copy in
 * ./parseargs.mjs instead of tools/lib/harness.mjs, so this runs without pulling
 * in playwright. No other changes.
 *
 * We evaluated the Thrixel asset-generation workflow and did not adopt it. This
 * measurement tooling we did adopt, and it earned its place in the first run by
 * proving a "visual improvement" we had already shipped was blowing out 20% of
 * the frame.
 */
/**
